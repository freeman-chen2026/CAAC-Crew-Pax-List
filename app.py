...
# ---------- 创建选项卡 ----------
tab1, tab2 = st.tabs(["📋 功能1：备案表生成", "🌐 功能2：世界时行程"])
...
with tab2:
    ...
    if uploaded_file_2 is not None:
        try:
            ...
            sorted_plans = sort_plans(plans)

            st.subheader("📋 生成的飞行计划")
            if len(sorted_plans) == 1:
                reg, text = list(sorted_plans.items())[0]
                st.code(text, language='text')
                st.caption("💡 选中上方文本，Ctrl+C 复制")
            else:
                for reg, text in sorted_plans.items():
                    st.markdown(f"**飞机注册号：{reg}**")
                    st.code(text, language='text')
                    st.markdown("---")
            ...
            for reg, text in sorted_plans.items():
                full_text += f"{text}\n\n"
            st.text_area("📦 全部计划合并（可直接复制）", full_text, height=250)
    ...
    st.markdown("---")
    st.caption("🛠️ 工具说明：日期/时间按北京时间（UTC+8）自动转换为世界时（Z）。")
